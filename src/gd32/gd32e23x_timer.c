...
static inline uint32_t timer_get(void) { return TIMER_CNT(TIMx); }
...
void timer_kick(void) { timer_set(timer_get() + 50); }
...
uint32_t __always_inline timer_read_time(void) {
...
void timer_reset(void) { sched_add_timer(&wrap_timer); }
...
void __aligned(16) TIMx_IRQHandler(void) {
  irq_disable();
