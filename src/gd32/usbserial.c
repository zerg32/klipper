...
static int_fast8_t usb_write_packet(uint32_t ep, const void *data,
                                    uint_fast8_t len);

static int_fast8_t usb_read_packet(uint32_t ep, void *data,
...
void usbserial_shutdown(void) {
  USB->ctrl &=
      ~(USB_SOF_INT | USB_RST_INT | USB_SP_INT | USB_WK_INT | USB_TC_INT);
}
